...
import { isTVar, makeFreshTVar, parseTExp, unparseTExp, TExp, NeverTExp, makeBoolTExp } from './TExp';
...
const parseProcExp = (vars: Sexp, rest: Sexp[]): Result<ProcExp> => {
    if (isArray(vars)) {
        ...
        const returnTE = rest[0] === ":" ? rest[1] === 'is?' ? makeOk(makeBoolTExp()) : parseTExp(rest[1]) : makeOk(makeFreshTVar());
