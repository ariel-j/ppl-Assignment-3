...
import { isTVar, makeFreshTVar, parseTExp, unparseTExp, TExp, NeverTExp, makeBoolTExp, makePredTExp } from './TExp';
...
const parseProcExp = (vars: Sexp, rest: Sexp[]): Result<ProcExp> => {
    if (isArray(vars)) {
        ...
        const returnTE = rest[0] === ":" ? rest[1] === 'is?' ? bind(parseTExp(rest[2]),(te : TExp) => makeOk(makePredTExp(te))) : parseTExp(rest[1]) : makeOk(makeFreshTVar());
